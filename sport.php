...
// On force le filtre sur la catégorie "Sport"
$requete = $bdd->prepare("SELECT * FROM evenements WHERE categorie = 'Sport' ORDER BY date_evenement ASC");
$requete->execute();
...
<head>
    <meta charset="UTF-8">
    <title>Équipes Sportives - OmnesEvent</title>
...
<nav class="menu-navigation">
    ...
    <a href="sport.php" class="active">Équipes sportives</a>
    <a href="culture.php">Asso's culturelles</a>
</nav>

<div class="page-header">
    <h1>🏆 Équipes Sportives</h1>
    <p>Rejoignez les tournois, entraînements et événements sportifs d'Omnes.</p>
</div>

<section class="liste-evenements">
    <?php if (count($evenements) === 0): ?>
        <p style="text-align: center; width: 100%; color: #5a5f8a;">Aucun événement sportif de prévu pour le moment.</p>
    <?php endif; ?>

    <?php foreach ($evenements as $evenement) : ?>
        <div class="card-evenement">
            <?php if (!empty($evenement["image_evenement"])) : ?>
                <img src="<?= htmlspecialchars($evenement["image_evenement"]) ?>" alt="Image événement" class="image-evenement">
            <?php else : ?>
                <div class="image-placeholder">Aucun visuel</div>
            <?php endif; ?>

            <div class="infos-evenement">
                <span class="badge-categorie cat-sport"><?= htmlspecialchars($evenement["categorie"]) ?></span>
